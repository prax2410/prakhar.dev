						...
						>
							...
							{index === 1 && (
								<div className="p-4 rounded-xl bg-zinc-50 dark:bg-zinc-800/40 border border-zinc-200/70 dark:border-zinc-700/60 flex flex-col lg:flex-row lg:items-center justify-between gap-3 text-xs">
									<div className="flex items-start gap-3 min-w-0">
										...
										<div className="space-y-0.5 min-w-0">
											<div className="font-semibold text-zinc-900 dark:text-white">
												Full-Stack Development & IIoT
												Integrations
											</div>
											<p className="text-zinc-600 dark:text-zinc-400 font-normal leading-relaxed">
												12 production projects
												independently designed &
												deployed across frontend, REST
												APIs, PostgreSQL & hardware
												protocols
											</p>
										</div>
									</div>
							...
							<ul className="space-y-2 text-xs sm:text-sm text-zinc-600 dark:text-zinc-300 leading-relaxed">
								{item.highlights.map((point, idx) => (
									...
									>
										<span className="text-zinc-400 dark:text-zinc-600 font-mono select-none">
											—
										</span>
										<span>{point}</span>
									</li>
							...
							<div className="pt-1 flex flex-wrap items-center gap-1.5 font-mono text-xs">
								<span className="text-zinc-400 font-sans text-xs mr-1">
									Stack:
								</span>
								{item.tech.map((t, idx) => (
									<span
